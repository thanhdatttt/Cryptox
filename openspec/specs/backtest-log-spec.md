...
- Evaluation metric calculation, score policy, or Top-10 admission rules;
  these are consumed from Evaluation and Leaderboard public APIs.
...
that supplies them.
 ...
 `BacktestQueueJob`, `BacktestQueueReturn`, or module ownership rules. §§4.1,
 4.2.1, and 4.4 define the additive mapping: REST/application adapters may
...
- **Idempotency key bounds:** a Manual `submissionIdempotencyKey` is a
 ...
 - **Manual benchmark selection:** the Manual screen may present benchmark
  ...
  `initialCapital`, finite non-negative `feeRatePercent`, and the MVP literal
   `slippageBps = 5`. It requires a bounded idempotency key and immutable
   request digest, then calls Market Data's idempotent
   `createOrGetSealedSnapshot` for the exact pair/range, persists both
   `datasetSnapshotId` and its content hash, and seals the scope before
   returning the scope ID. Market Data and PostgreSQL are separate resource
   managers, so this is an idempotent two-step saga rather than a pretend
   distributed transaction: retry/reconciliation reuses the same snapshot and
   scope, and a conflicting body returns `409`. `startManual` accepts only that
 ...
 - **Manual strategy validation:** definitions are created or verified through
   the Strategy public API first. Backtesting then persists only immutable
   definition IDs, versions, implementation hashes, and the Composite
   reference in its own Candidate transaction; it never writes Strategy-owned
   tables or pretends to share a cross-module database transaction. Unknown,
   duplicate, mismatched, or conflicting IDs reject with a typed `400`/`409`.
...
- **Evaluation policy `MVP_EVALUATION_V1`:** over the deterministically ordered
  closed Trades ordered by `(entryTime ASC, sequence ASC, id ASC)`,
  ...
  `resultPercent / 100`, arithmetic mean, sample standard deviation, and
...
- **Decimal policy `MVP_DECIMAL_HALF_UP_V1`:** use exact decimal arithmetic;
 ...
 - **Fill policy `MVP_OHLC_STOP_FIRST_V1`:** candles are processed in UTC
  ...
  take-profit when `high >= takeProfit`; for `SHORT`, stop-loss triggers when
  `high >= stopLoss` and take-profit when `low <= takeProfit`. If both trigger
...
- **Loop observability ownership:** Backtesting supplies per-Candidate
  ...
  `FAILED` Candidates; `failedAttemptCount` counts failed Attempts attached to
...
// asks Market Data to createOrGet a sealed DatasetSnapshotRef before mapping
// this request to the main-compatible CreateLeaderboardScopeCommand below.
...
sequenceDiagram
    ...
    BC->>MD: createOrGet sealed snapshot(pair, timeframe, [from,to), requestHash)
    MD-->>BC: datasetSnapshotId + content hash
    BC->>BC: map DatasetSnapshotRef + runtime refs to main-compatible scope command
    BC->>PG: persist immutable LeaderboardScope referencing snapshot
    PG-->>BC: leaderboardScopeId + scope summary
...
before the Candidate-locked transition. A generic `failed` callback, an old
evidence object, or a job merely being `active` is insufficient to close an
Attempt. The watchdog and replacement worker use this same predicate before
allocating the next generation.
...
Backtesting does not write `search_runs` or Search-owned counters. After its
Candidate/Experiment transaction commits, it invokes the direct post-commit
`onCandidateFinished(searchRunId)` callback. Search then obtains the public
Candidate summary and updates its own `LoopStatus` projection; a lost callback
is repaired by Search startup/periodic reconciliation.
...
For Search cancellation, Search first commits its own `SearchRun` transition
and asks Backtesting to cancel the non-terminal Candidate IDs through the
opaque `CancellationUnitOfWork`. After that commit, Search calls the
...
durable, and startup reconciliation can retry the cleanup.
...
| Invalid lifecycle transition | A worker, reconciler, cancellation, or completion callback observes a state not allowed by §2.4 | Lock/reload, reject or no-op the transition, and leave the durable Candidate state unchanged. |
...
// Additive extension of branch-main EvaluationMetrics. Existing metric names
// and `number` types remain unchanged; totalProfitAmount is the new absolute
// P&L field required by the Backtest Log.
export interface EvaluationMetrics {
  ...
  maxDrawdownPercent: number; // non-negative loss magnitude
...
{ pair, timeframe, datasetRange: { from, to }, initialCapital,
  feeRatePercent, slippageBps: 5, scoreFormulaId, sentimentSnapshotId? }
...
200 OK
CandidateProgress[] // apps/backend composition from Backtesting projections
...
All metric numbers must be finite. `totalProfitAmount` is the sum of closed
...
### 4.2 Durable log ownership and mapping

| Durable record | Owner | Required audit meaning |
|---|---|---|
| `leaderboard_scopes` | `modules/backtesting` setup facade / Leaderboard scope owner | Immutable pair, timeframe, `[from,to)` range, `datasetSnapshotId` plus content hash, capital, fee, fixed 5-bps slippage, score formula, decimal policy, and evaluation policy selected for comparison. |
...
```sql
-- These columns already exist in the branch-main baseline and are listed here
-- as compatibility requirements, not migrations:
--   name TEXT NOT NULL, version INT > 0,
--   dataset_snapshot_id UUID, sentiment_snapshot_id UUID,
--   worker_runtime_version/sha256, evaluation_runtime_version/sha256,
--   initial_capital NUMERIC > 0, fee_rate_percent NUMERIC >= 0,
--   slippage_bps INT >= 0, score_formula_id UUID.
-- The additive migration below must validate/preserve those columns rather
-- than re-add them.
...
transaction as the immutable scope row after Market Data returns its sealed
snapshot. A repeated scope key with the same digest returns the existing scope;
a different digest returns `409` and writes nothing. If the process fails after
Market Data seals the snapshot but before PostgreSQL commits, retrying the same
key calls Market Data's create-or-get operation and then repairs the missing
scope mapping. An orphaned sealed snapshot is never attached to a different
request and may be garbage-collected only by Market Data reconciliation.
...
| Sealed snapshot is missing/incomplete after a worker claim | `FAILED`, `failure_category = INFRASTRUCTURE`, `failure_code = MISSING_SNAPSHOT`, `failure_retryable = false` | `TERMINAL_FAILURE_PENDING` with `failureKind = INFRASTRUCTURE`; completion finalizes `FAILED`. |
...
- [ ] Scope creation requires a bounded `Idempotency-Key` and request SHA-256;
  ...
  scope commit is repaired by create-or-get/reconciliation without attaching
...
- [ ] Attempt failure persistence uses exactly the §4.2.1 category/code mapping:
...
- [ ] Fixture tests for `MVP_OHLC_STOP_FIRST_V1` cover next-open entry, LONG and
  SHORT trigger direction, gap-at-open fills, both stop/take-profit triggers in
  one candle (stop wins), strategy-close fills, forced close at range end, and
  the explicit final-candle entry/`RANGE_END` exception, and no same-candle
  re-entry.
...
- [ ] `summarizeSearchCandidates` returns only active Candidate projections
  plus `queuedCount`, `runningCount`, and `candidatesTested`; active rows have
  deterministic ordering. Search owns `maxInFlight` and never admits more
  active rows than its own persisted limit.
