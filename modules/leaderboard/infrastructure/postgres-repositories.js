...
class PostgresLeaderboardEntryRepository {
    ...
    async getActiveTopK(scopeId, limit, unitOfWork) { const client = unitOfWork?.query ? { query: (text, values) => unitOfWork.query(text, values) } : this.pool; const result = await client.query("SELECT id, experiment_result_id, leaderboard_scope_id, score_formula_id, score, added_at FROM leaderboard_entries WHERE leaderboard_scope_id = $1 AND active = TRUE ORDER BY score DESC, added_at ASC, id ASC LIMIT $2", [scopeId, limit]); return result.rows.map(entry); }
    async getByExperimentResultId(experimentResultId, unitOfWork) { const client = unitOfWork?.query ? { query: (text, values) => unitOfWork.query(text, values) } : this.pool; const result = await client.query("SELECT id, experiment_result_id, leaderboard_scope_id, score_formula_id, score, added_at FROM leaderboard_entries WHERE experiment_result_id = $1", [experimentResultId]); return result.rows[0] ? entry(result.rows[0]) : undefined; }
    async insert(input, unitOfWork) { const client = unitOfWork?.query ? { query: (text, values) => unitOfWork.query(text, values) } : this.pool; const result = await client.query("INSERT INTO leaderboard_entries (id, experiment_result_id, leaderboard_scope_id, score_formula_id, score, added_at, active) VALUES ($1, $2, $3, $4, $5, $6, TRUE) RETURNING id, experiment_result_id, leaderboard_scope_id, score_formula_id, score, added_at", [`leaderboard-entry-${crypto.randomUUID()}`, input.experimentResultId, input.leaderboardScopeId, input.scoreFormulaId, input.score, input.addedAt]); return entry(result.rows[0]); }
    async deactivate(entryId, unitOfWork) { const client = unitOfWork?.query ? { query: (text, values) => unitOfWork.query(text, values) } : this.pool; await client.query("UPDATE leaderboard_entries SET active = FALSE WHERE id = $1", [entryId]); }
}
