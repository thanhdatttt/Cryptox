...
describe("PostgresNewsRepository", () => {
  it("uses URL-based durable deduplication and parameterized values", async () => {
    ...
    await expect(repository.insert({ id: row.id, title: row.title, content: row.content, source: row.source, publishedAt: row.published_at, crawledAt: row.crawled_at, relatedCoins: ["BTC"], url: row.url }, "user-1")).resolves.toMatchObject({ url: row.url });
    expect(calls[0]!.text).toContain("ON CONFLICT (user_id, url)");
    expect(calls[0]!.values).toContain(row.url);
    expect(calls[0]!.values).toContain("user-1");

    await repository.readAll("user-1");
    expect(calls[1]!.text).toContain("WHERE user_id = $1");
    expect(calls[1]!.values).toEqual(["user-1"]);
  });
});
