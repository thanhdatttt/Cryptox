...
exports.COINDESK_RSS_FEED_URL = void 0;
exports.createCoinDeskRssProvider = createCoinDeskRssProvider;
...
exports.COINDESK_RSS_FEED_URL = "https://www.coindesk.com/arc/outboundfeeds/rss/";
...
const decodeXmlEntities = (value) => value
    ...
    .replace(/&amp;/gi, "&")
    .replace(/&nbsp;?/gi, " ");
const normalizedText = (value) => decodeXmlEntities(value.replace(/<!\[CDATA\[([\s\S]*?)\]\]>/g, "$1"))
    .replace(SCRIPT_OR_STYLE, "")
    .replace(TAG, " ")
    .replace(/\s+/g, " ")
    .trim();
const escapeRegExp = (value) => value.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
const elementValues = (block, names) => {
...
const linkValues = (block) => {
    ...
    for (const match of block.matchAll(pattern)) {
        ...
        if (value)
            result.push(normalizedText(value));
    }
    return result;
};
const firstText = (block, names) => {
...
const normalizeRelatedCoins = (values) => {
    const coins = [];
    for (const value of values.flatMap((entry) => entry.split(/[,|;]/))) {
        ...
        if (!candidate || categoryNoise.has(candidate) || !/^[A-Z0-9]{2,12}$/.test(candidate))
            continue;
        if (!coins.includes(candidate))
            coins.push(candidate);
    }
    return coins;
};
const observeInvalidEntry = (observability) => {
    try {
...
const normalizeEntry = (block, crawledAt) => {
    ...
    if (!title || !content || !url || !publishedAt)
        return undefined;
    try {
        return (0, rules_1.validateNewsItem)({
            id: (0, node_crypto_1.createHash)("sha256").update(url, "utf8").digest("hex").slice(0, 24),
            title,
            content,
            source: SOURCE_NAME,
            publishedAt,
...
const parseFeed = (xml, crawledAt, observability) => {
    const items = [];
    for (const match of xml.matchAll(ENTRY)) {
        ...
        if (item)
            ...
            observeInvalidEntry(observability);
    }
    return items;
};
function createCoinDeskRssProvider(options = {}) {
    ...
    return {
        ...
        async fetch() {
            ...
            if (!crawledAt)
                throw new Error("COINDESK_RSS_INVALID_CRAWL_TIME");
            const response = await client(exports.COINDESK_RSS_FEED_URL, {
                headers: { accept: "application/rss+xml, application/xml, text/xml;q=0.9" },
            });
            if (!response.ok)
                throw new Error(`COINDESK_RSS_HTTP_${response.status}`);
            const xml = await response.text();
            return parseFeed(xml, crawledAt, options.observability);
        },
    };
}
function createConfiguredNewsProviders(input = {}) {
