...
class PostgresSearchRunRepository {
    ...
    async withRunLock(ownerUserId, id, operation) {
        if (!this.pool.connect)
            return operation(await this.getByOwner(ownerUserId, id));
        const client = await this.pool.connect();
        await client.query("BEGIN", []);
        let closed = false;
        const unitOfWork = { kind: "CANCELLATION", id: `search-fill-${(0, node_crypto_1.randomUUID)()}`, query: (text, values) => client.query(text, values), run: async (callback) => callback(), onRollback: () => undefined, commit: async () => { if (!closed) {
        ...
        try {
            const result = await client.query(`SELECT ${this.fields()} FROM search_runs WHERE id = $1 AND owner_user_id = $2 FOR UPDATE`, [id, ownerUserId]);
