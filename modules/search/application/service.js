...
const createRun = (searchRunId, config, now) => ({
...
function createSearchModule(dependencies = createInMemorySearchDependencies()) {
    ...
    const fill = async (searchRunId) => {
        ...
        fills.set(searchRunId, current.finally(() => { if (fills.get(searchRunId) === current)
            fills.delete(searchRunId); }));
        return current;
    };
    return {
        start: async (config) => { validateStopCondition(config.stopCondition); if (!Number.isInteger(config.maxInFlight) || config.maxInFlight <= 0 || !config.leaderboardScopeId)
            throw new Error("INVALID_SEARCH_CONFIG"); const run = createRun(idGenerator(), config, dependencies.clock.now()); await dependencies.searchRunRepository.insert(run); await fill(run.searchRunId); return { searchRunId: run.searchRunId }; },
        pause: async (id) => { const run = await load(id); if (run.state === "RUNNING") {
        ...
        resume: async (id) => { const run = await load(id); if (run.state === "FAILED")
        ...
        cancel: async (id) => { const run = await load(id); if (run.state === "CANCELLED" || run.state === "FAILED")
