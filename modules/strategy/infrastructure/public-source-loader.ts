...
const boundedOption = (value: number | undefined, fallback: number, integer = false, allowZero = false): number => {
  if (value === undefined || !Number.isFinite(value) || value < 0 || (!allowZero && value === 0) || (integer && !Number.isInteger(value))) return fallback;
  return value;
};
...
const blockedIpv6 = (value: string): boolean => {
  ...
  if (normalized === "::" || normalized === "::1" || /^fe[89ab]/.test(normalized) || /^(fc|fd)/.test(normalized) || normalized.startsWith("ff")) return true;
  const mapped = normalized.match(/^::ffff:(\d+\.\d+\.\d+\.\d+)$/);
  return mapped ? blockedIpv4(mapped[1]!) : false;
};

const blockedAddress = (value: string): boolean => {
...
const assertPublicUrl = async (value: string, resolve: PublicStrategySourceLoaderOptions["lookup"], timeoutMs: number): Promise<URL> => {
  let url: URL;
  try { url = new URL(value); } catch { throw new Error("STRATEGY_SOURCE_INVALID_URL"); }
  if (!(["http:", "https:"].includes(url.protocol)) || url.username || url.password || !url.hostname) throw new Error("STRATEGY_SOURCE_UNSAFE");
  const hostname = url.hostname.replace(/^\[|\]$/g, "");
  if (blockedHostname(hostname)) throw new Error("STRATEGY_SOURCE_UNSAFE");
  const kind = isIP(hostname);
  if (kind !== 0 && blockedAddress(hostname)) throw new Error("STRATEGY_SOURCE_UNSAFE");
  if (kind === 0) {
    let addresses: Array<{ address: string }>;
    const resolver = resolve ?? (async (name: string, options: { all: true; verbatim: true }) => await lookup(name, options) as Array<{ address: string; family?: number }>);
    try { addresses = await withTimeout(resolver(hostname, { all: true, verbatim: true }), timeoutMs, "STRATEGY_SOURCE_TIMEOUT"); } catch (error) {
      if (error instanceof Error && error.message === "STRATEGY_SOURCE_TIMEOUT") throw error;
      throw new Error("STRATEGY_SOURCE_UNAVAILABLE");
    }
    if (!addresses.length || addresses.some(({ address }) => blockedAddress(address))) throw new Error("STRATEGY_SOURCE_UNSAFE");
  }
  return url;
};
...
const readBoundedBody = async (response: Response, maxBytes: number): Promise<string> => {
  ...
  if (Number.isFinite(length) && length > maxBytes) throw new Error("STRATEGY_SOURCE_TOO_LARGE");
  if (!response.body) {
    const value = await response.text();
    if (Buffer.byteLength(value, "utf8") > maxBytes) throw new Error("STRATEGY_SOURCE_TOO_LARGE");
    return value;
  }
  ...
  } finally { await reader.releaseLock(); }
...
const readableText = (body: string, contentType: string, maxCharacters: number): string => {
...
export function createPublicStrategySourceLoader(options: PublicStrategySourceLoaderOptions = {}): StrategySourceLoader {
    ...
    async load(inputUrl: string) {
      ...
      for (let redirect = 0; redirect <= maxRedirects; redirect += 1) {
        ...
        try {
          ...
          if (!response.ok) throw new Error("STRATEGY_SOURCE_UNAVAILABLE");
          const contentType = (response.headers.get("content-type") ?? "").toLowerCase();
          if (!(contentType.includes("text/html") || contentType.includes("application/xhtml+xml") || contentType.includes("text/plain"))) throw new Error("STRATEGY_SOURCE_UNSUPPORTED_CONTENT");
          ...
          if (sourceText.length < 20) throw new Error("STRATEGY_SOURCE_UNUSABLE");
          return { sourceText, canonicalUrl: current.toString() };
        } catch (error) {
          if (controller.signal.aborted) throw new Error("STRATEGY_SOURCE_TIMEOUT");
