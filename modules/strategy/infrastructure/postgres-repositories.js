...
const definition = (row) => ({ id: row.id, userId: row.user_id, logicalFamilyKey: row.logical_family_key, familyName: row.family_name ?? undefined, strategyName: row.strategy_name, implementationVersion: row.implementation_version, implementationSha256: row.implementation_sha256, version: row.version, parameters: object(row.parameters), createdAt: new Date(row.created_at).toISOString() });
const composite = (row) => ({ id: row.id, userId: row.user_id, logicalFamilyKey: row.logical_family_key, version: row.version, method: row.method, components: object(row.components), thresholds: row.thresholds === null ? undefined : object(row.thresholds), createdAt: new Date(row.created_at).toISOString() });
...
class PostgresStrategyDefinitionRepository {
    ...
    async list(ownerUserId) {
        const result = await this.pool.query("SELECT id, user_id, logical_family_key, family_name, strategy_name, implementation_version, implementation_sha256, version, parameters, created_at FROM strategy_definitions WHERE user_id = $1 ORDER BY created_at ASC, id ASC", [ownerUserId]);
        return result.rows.map(definition);
    }
    async listByIds(ownerUserId, ids) {
        if (ids.length === 0)
            return [];
        const result = await this.pool.query("SELECT id, user_id, logical_family_key, family_name, strategy_name, implementation_version, implementation_sha256, version, parameters, created_at FROM strategy_definitions WHERE user_id = $1 AND id = ANY($2::text[])", [ownerUserId, ids]);
    ...
    async listByLogicalFamily(ownerUserId, logicalFamilyKey) {
        const result = await this.pool.query("SELECT id, user_id, logical_family_key, family_name, strategy_name, implementation_version, implementation_sha256, version, parameters, created_at FROM strategy_definitions WHERE user_id = $1 AND logical_family_key = $2 ORDER BY version ASC", [ownerUserId, logicalFamilyKey]);
        return result.rows.map(definition);
    }
    async exists(id) {
...
class PostgresCompositeDefinitionRepository {
    ...
    async insert(ownerUserId, value) {
        const result = await this.pool.query("INSERT INTO composite_strategy_definitions (id, user_id, logical_family_key, version, method, components, thresholds, created_at) VALUES ($1, $2, $3, $4, $5, $6::jsonb, $7::jsonb, $8) RETURNING id, user_id, logical_family_key, version, method, components, thresholds, created_at", [value.id, ownerUserId, value.logicalFamilyKey, value.version, value.method, JSON.stringify(value.components), JSON.stringify(value.thresholds ?? null), value.createdAt]);
        return composite(result.rows[0]);
    }
    async list(ownerUserId) {
        const result = await this.pool.query("SELECT id, user_id, logical_family_key, version, method, components, thresholds, created_at FROM composite_strategy_definitions WHERE user_id = $1 ORDER BY created_at ASC, id ASC", [ownerUserId]);
        return result.rows.map(composite);
    }
    async get(ownerUserId, id) {
        const result = await this.pool.query("SELECT id, user_id, logical_family_key, version, method, components, thresholds, created_at FROM composite_strategy_definitions WHERE user_id = $1 AND id = $2 LIMIT 1", [ownerUserId, id]);
        return result.rows[0] ? composite(result.rows[0]) : undefined;
    }
    async listByLogicalFamily(ownerUserId, logicalFamilyKey) {
        const result = await this.pool.query("SELECT id, user_id, logical_family_key, version, method, components, thresholds, created_at FROM composite_strategy_definitions WHERE user_id = $1 AND logical_family_key = $2 ORDER BY version ASC", [ownerUserId, logicalFamilyKey]);
        return result.rows.map(composite);
    }
}
...
const createPostgresStrategyDependencies = (pool) => ({
    artifactResolver: {
        ...
        resolveSync: (strategyName, implementationSha256) => plugins_1.builtInFactories.find((item) => item.descriptor.name === strategyName && item.descriptor.implementationSha256 === implementationSha256),
    },
