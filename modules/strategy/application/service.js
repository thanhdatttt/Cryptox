...
function createStrategyModule(dependencies = createInMemoryStrategyDependencies()) {
    ...
    const ownedDefinitions = (userId) => [...definitions.values()].filter((item) => item.ownerUserId === userId).map((item) => item.value);
    const ownedComposites = (userId) => [...composites.values()].filter((item) => item.ownerUserId === userId).map((item) => item.value);
