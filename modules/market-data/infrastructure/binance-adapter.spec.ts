...
describe("Binance market-data adapter", () => {
  ...
  it("normalizes combined-stream trades and candles at the adapter boundary", async () => {
    let socket!: { onmessage: ((event: { data: string }) => void) | null; onclose: (() => void) | null; onerror: (() => void) | null; close(): void };
    const adapter = createBinanceMarketDataAdapter({ fetchFn: async () => ({ ok: true, status: 200, json: async () => [] }), webSocketFactory: () => (socket = { onmessage: null, onclose: null, onerror: null, close: () => undefined }) });
