...
export class BacktestingService implements BacktestLogApi {
  ...
  private async validateStrategyReferences(ownerUserId: string, command: StartManualBacktestCommand | SubmitSearchCandidateCommand): Promise<{ strategyDefinitions: StrategyDefinition[]; compositeDefinition: CompositeStrategyDefinition }> {
  ...
  async listAttemptTrades(auth: AuthContext, attemptId: string, page: TradePageRequest): Promise<TradePage> { await this.readAttempt(auth, attemptId); return this.pageTrades(await this.deps.repository.listTrades(attemptId), page); }
  ...
  async listExperimentTrades(auth: AuthContext, experimentId: string, page: TradePageRequest): Promise<TradePage> { const experiment = await this.readExperimentSummary(auth, experimentId); return this.pageTrades(experiment.trades, page); }
