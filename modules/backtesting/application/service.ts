...
export class InMemoryBacktestQueue implements BacktestQueuePort {
  ...
  async enqueue(job: BacktestQueueJob): Promise<void> { if (!this.jobs.has(job.jobId)) this.jobs.set(job.jobId, clone(job)); }
...
export class InMemoryBacktestingRepository implements BacktestingRepository {
  ...
  async failWorkerAttempt(input: { candidate: StoredCandidate; attempt: BacktestAttemptAudit; fenceToken: string; retrying: boolean; now: string; error: string }): Promise<void> {
    ...
    const failed: BacktestAttemptAudit = { ...attempt, status: "FAILED", completedAt: input.now, failureCategory: input.retrying ? "RETRYABLE" : "INFRASTRUCTURE", failureCode: input.error, errorMessage: input.error, leaseExpiresAt: undefined };
...
export class BacktestingService implements BacktestLogApi {
  ...
  async listBenchmarkScopes(options: Required<BacktestReadOptions>): Promise<BenchmarkScopeSummary[]> { if (!options.ownerUserId?.trim()) invalid("INVALID_BACKTEST_SCOPE_LIST"); return this.deps.repository.listScopesByOwner(options.ownerUserId); }
  ...
  async status(candidateId: string, options?: BacktestReadOptions): Promise<CandidateProgress> { const candidate = await this.candidate(candidateId, options); return this.progress(candidate, await this.deps.repository.listAttempts(candidateId)); }
  async summarizeSearchCandidates(searchRunId: string): Promise<SearchCandidateSummary> { const candidates = await this.deps.repository.listCandidatesBySearchRun(searchRunId); const attemptsByCandidate = await Promise.all(candidates.map(async (candidate) => [candidate, await this.deps.repository.listAttempts(candidate.candidateId)] as const)); const active = attemptsByCandidate.filter(([candidate]) => !terminal(candidate.status)).map(([candidate, attempts]) => this.progress(candidate, attempts)); const attempts = attemptsByCandidate.flatMap(([, candidateAttempts]) => candidateAttempts); const failed = candidates.filter((candidate) => candidate.status === "FAILED" || candidate.status === "TERMINAL_FAILURE_PENDING"); return { searchRunId, active, queuedCount: candidates.filter((candidate) => candidate.status === "QUEUED").length, runningCount: candidates.filter((candidate) => ["BACKTESTING", "RETRY_WAIT", "PROCESSING_RESULT"].includes(candidate.status)).length, candidatesTested: candidates.length, failedCandidateCount: failed.length, retryExhaustedCandidateCount: failed.filter((candidate) => candidate.failureKind === "RETRY_EXHAUSTED").length, infrastructureFailureCandidateCount: failed.filter((candidate) => candidate.failureKind === "INFRASTRUCTURE").length, completionProcessingFailureCandidateCount: failed.filter((candidate) => candidate.failureKind === "COMPLETION_PROCESSING").length, failedAttemptCount: attempts.filter((attempt) => attempt.status === "FAILED").length, averageBacktestDurationMs: null }; }
  async listSearchCandidates(searchRunId: string, page: SearchCandidatePageRequest): Promise<SearchCandidatePage> { if (!Number.isInteger(page.limit) || page.limit < 1) invalid("INVALID_PAGE"); const items = (await this.deps.repository.listCandidatesBySearchRun(searchRunId)).sort((left, right) => left.createdAt.localeCompare(right.createdAt)); const offset = page.cursor ? Number(page.cursor) : 0; if (!Number.isInteger(offset) || offset < 0) invalid("INVALID_PAGE"); const selected = items.slice(offset, offset + page.limit); return { items: await Promise.all(selected.map(async (candidate) => this.progress(candidate, await this.deps.repository.listAttempts(candidate.candidateId)))), nextCursor: offset + page.limit < items.length ? String(offset + page.limit) : undefined }; }
  async cancelSearchCandidates(searchRunId: string): Promise<{ candidateIds: string[] }> { const candidates = await this.deps.repository.listCandidatesBySearchRun(searchRunId); const cancelled: string[] = []; for (const candidate of candidates) if (!terminal(candidate.status)) { candidate.status = "CANCELLED"; candidate.activeFenceToken = undefined; candidate.activeLeaseExpiresAt = undefined; candidate.updatedAt = this.now(); await this.deps.repository.updateCandidate(candidate); await this.deps.repository.markDispatchCancelled(candidate.queueJobId, candidate.updatedAt); cancelled.push(candidate.candidateId); } return { candidateIds: cancelled }; }
  async cancelManualCandidate(candidateId: string, unitOfWork: import("../domain/contracts").CancellationUnitOfWork, options?: BacktestReadOptions): Promise<void> { if (unitOfWork.kind !== "CANCELLATION" || !unitOfWork.id) invalid("INVALID_CANCELLATION"); const candidate = await this.candidate(candidateId, options); if (candidate.origin !== "MANUAL") invalid("BACKTEST_CANDIDATE_NOT_MANUAL"); if (!terminal(candidate.status)) { candidate.status = "CANCELLED"; candidate.activeFenceToken = undefined; candidate.activeLeaseExpiresAt = undefined; candidate.updatedAt = this.now(); await this.deps.repository.updateCandidate(candidate); await this.deps.repository.markDispatchCancelled(candidate.queueJobId, candidate.updatedAt); } }
  ...
  async readExperimentVisualization(experimentId: string, page: ExperimentVisualizationPageRequest, options?: BacktestReadOptions): Promise<ExperimentVisualization> {
    if (!Number.isInteger(page.limit) || page.limit < 1) invalid("INVALID_VISUALIZATION_PAGE");
    const experiment = await this.readExperimentSummary(experimentId, options); const input = await this.snapshot(experiment.datasetSnapshot.id);
    const from = page.from ?? input.snapshot.range.from; const to = page.to ?? input.snapshot.range.to;
    if (!Number.isFinite(Date.parse(from)) || !Number.isFinite(Date.parse(to)) || Date.parse(from) < Date.parse(input.snapshot.range.from) || Date.parse(to) > Date.parse(input.snapshot.range.to) || Date.parse(from) >= Date.parse(to)) invalid("INVALID_VISUALIZATION_PAGE");
    ...
    const allCandles = input.candles.filter((candle) => candle.isClosed && candle.timestamp >= from && candle.timestamp < to); const candles = allCandles.slice(offset, offset + page.limit);
    const marker = (trade: Trade, kind: "ENTRY" | "STOP_LOSS" | "TAKE_PROFIT" | "EXIT", time: string, price: number): import("../domain/contracts").ExperimentVisualizationMarker => ({ id: `${trade.id}:${kind}`, tradeId: trade.id, sequence: trade.sequence, kind, time, price, highlighted: trade.id === page.highlightTradeId });
