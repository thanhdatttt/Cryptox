...
export interface CreateLeaderboardScopeCommand { name: string; datasetSnapshot: DatasetSnapshotRef; sentimentDatasetSnapshot?: SentimentDatasetSnapshotRef; initialCapital: number; feeRatePercent: number; slippageBps: number; warmupCapacityCandles?: number; scoreFormulaId: string; riskPolicy?: { stopLossPercent?: number; takeProfitPercent?: number }; workerRuntimeVersion: string; workerRuntimeSha256: string; evaluationRuntimeVersion: string; evaluationRuntimeSha256: string; }
export interface StartManualBacktestCommand { leaderboardScopeId: string; strategyDefinitions: StrategyDefinition[]; compositeDefinition: CompositeStrategyDefinition; maxAttempts: number; }
export interface SubmitSearchCandidateCommand extends StartManualBacktestCommand { searchRunId: string; iterationNumber: number; generatedBy: GeneratorType; }
...
export interface ReplayVerificationResult { experimentId: string; sourceAttemptId: string; status: "MATCH" | "MISMATCH" | "NON_REPLAYABLE"; comparedTradeCount: number; mismatches: Array<{ fieldPath: string; expected: string; actual: string }>; failureCode?: "MISSING_SNAPSHOT" | "IMPLEMENTATION_ARTIFACT_UNAVAILABLE"; }
