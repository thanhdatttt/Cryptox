...
The highest-authority source is the protected instructor assignment **“Crypto Strategy Lab – Đồ án cuối kỳ”**, 54 pages, at `docs/assignment/Crypto Strategy Lab – Đồ án cuối kỳ.pdf`. Its verified SHA-256 is `A7D877F4614AE4E78BBEFB3F271BF2022B50AFD5CB6C9CB0CAA16EF80845771B`; provenance is recorded in `docs/assignment/README.md`. This baseline interprets its Vietnamese normative wording as follows:
...
| CSL-R-LB-01 | Maintain a configurable Top-K leaderboard from evaluated backtests. K=10 may be a UI/default value, not an architectural invariant; any overall score must be explained and reproducible. | §§21–22, pp. 21–23; MVP §37, p. 41 | `leaderboard` (`openspec/specs/leaderboard/spec.md`) | UNIT, INTEGRATION, REPRO | SCAFFOLDED; fixed-K/source inconsistencies need later reconciliation |
...
| CSL-O-07 | Authentication, users, ownership/authorization, multi-tenancy/tenant isolation, AI/LLM strategy authoring, and mandatory LLM news crawling. | Approved Stage 2 project decision; not assignment MVP requirements | DEFERRED; must not appear normatively in active docs/specs. |
