...
| I-02 | IN_PROGRESS | 6 | YES | INS-191 fresh same-directory Manager; exactly one hidden worker authorized | `MVP_IMPLEMENTATION` / reviewed source-business checkpoint `9c0fe4c`; pre-existing `.codex/config.toml` excluded | `REVIEW -> READY -> IN_PROGRESS` under INS-191 / DEC-112; exactly one worker is now authorized for the two simulator paths. No other task state changed. |
