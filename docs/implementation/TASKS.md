...
| I-02 | REVIEW | 6 | YES | INS-150 Manager / exactly one fresh hidden internal implementation worker Russell `01a05875-42d3-7f03-982a-c8a768b6c4a9` (completed) | `MVP_IMPLEMENTATION` / containing INS-150 checkpoint commit; exact two-file source/test scope integrated | `REVIEW -> READY -> IN_PROGRESS -> REVIEW` under INS-150; receiver-preserving market history fix and regression accepted; live remote history boundary PASS; broader Auth/realtime/News/LLM/OpenSpec/clean-install evidence remains BLOCKED or UNVERIFIED; no downstream task is authorized |
