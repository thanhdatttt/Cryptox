...
- `docker --version`: could not run because Docker CLI is not installed in this environment. No PostgreSQL container was available to apply migration `004` here; repository SQL behavior is covered by focused tests, and live migration/application remains a Docker-backed final gate.
...
- No product-scope blocker is known. The next required Backtesting work is durable queue dispatch/consumption/recovery; Search and Leaderboard persistence/flows remain incomplete after that.
