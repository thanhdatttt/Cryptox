...
const normalizeCapabilities = (value: unknown): MarketCapabilities => { const raw = dto<Record<string, unknown>>(value, "market capabilities"); if (!Array.isArray(raw.pairs) || !Array.isArray(raw.timeframes) || raw.pairs.some((pair) => typeof pair !== "string" || !pair.trim()) || raw.timeframes.some((timeframe) => !validTimeframe(timeframe))) throw new ApiError(502, "Backend returned an invalid market capabilities response.", "INVALID_DTO"); return { provider: textField(raw.provider, "market capabilities"), pairs: raw.pairs as string[], timeframes: raw.timeframes as Timeframe[], ...(isRecord(raw.policyDefaults) ? { policyDefaults: raw.policyDefaults as MarketCapabilities["policyDefaults"] } : {}) }; };
