...
exports.up = (pgm) => {
  ...
  pgm.addConstraint(
    "backtest_replay_verifications",
    "backtest_replay_counts_valid",
    {
      check:
        "(compared_trade_count IS NULL OR compared_trade_count >= 0) AND " +
        "(total_mismatch_count IS NULL OR total_mismatch_count >= 0)",
    },
  );
  pgm.addConstraint(
    "backtest_replay_verifications",
    "backtest_replay_failure_valid",
    {
      check:
  ...
  pgm.addConstraint(
    "backtest_replay_verifications",
    "backtest_replay_terminal_shape",
    {
      check:
        "(status IN ('QUEUED','RUNNING') AND completed_at IS NULL AND failure_code IS NULL) OR " +
        "(status = 'NON_REPLAYABLE' AND failure_code IS NOT NULL AND completed_at IS NOT NULL) OR " +
        "(status IN ('MATCH','MISMATCH') AND compared_trade_count IS NOT NULL AND " +
        "total_mismatch_count IS NOT NULL AND truncated IS NOT NULL AND completed_at IS NOT NULL)",
    },
  );
