...
- Search status reports `averageBacktestDurationMs` only from completed Attempts with valid start/completion timestamps; `—` means there is no valid sample, not zero milliseconds. Use the [scale benchmark procedure](docs/evidence/backtest-scale-benchmark.md) before making a throughput claim.
